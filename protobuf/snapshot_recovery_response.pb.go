// Code generated by protoc-gen-go. DO NOT EDIT.
// source: snapshot_recovery_response.proto

package protobuf

import proto "github.com/golang/protobuf/proto"
import fmt "fmt"
import math "math"

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion2 // please upgrade the proto package

type SnapshotRecoveryResponse struct {
	Term                 uint64   `protobuf:"varint,1,opt,name=Term" json:"Term,omitempty"`
	Success              bool     `protobuf:"varint,2,opt,name=Success" json:"Success,omitempty"`
	CommitIndex          uint64   `protobuf:"varint,3,opt,name=CommitIndex" json:"CommitIndex,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *SnapshotRecoveryResponse) Reset()         { *m = SnapshotRecoveryResponse{} }
func (m *SnapshotRecoveryResponse) String() string { return proto.CompactTextString(m) }
func (*SnapshotRecoveryResponse) ProtoMessage()    {}
func (*SnapshotRecoveryResponse) Descriptor() ([]byte, []int) {
	return fileDescriptor_snapshot_recovery_response_01c34a964a4060da, []int{0}
}
func (m *SnapshotRecoveryResponse) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_SnapshotRecoveryResponse.Unmarshal(m, b)
}
func (m *SnapshotRecoveryResponse) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_SnapshotRecoveryResponse.Marshal(b, m, deterministic)
}
func (dst *SnapshotRecoveryResponse) XXX_Merge(src proto.Message) {
	xxx_messageInfo_SnapshotRecoveryResponse.Merge(dst, src)
}
func (m *SnapshotRecoveryResponse) XXX_Size() int {
	return xxx_messageInfo_SnapshotRecoveryResponse.Size(m)
}
func (m *SnapshotRecoveryResponse) XXX_DiscardUnknown() {
	xxx_messageInfo_SnapshotRecoveryResponse.DiscardUnknown(m)
}

var xxx_messageInfo_SnapshotRecoveryResponse proto.InternalMessageInfo

func (m *SnapshotRecoveryResponse) GetTerm() uint64 {
	if m != nil {
		return m.Term
	}
	return 0
}

func (m *SnapshotRecoveryResponse) GetSuccess() bool {
	if m != nil {
		return m.Success
	}
	return false
}

func (m *SnapshotRecoveryResponse) GetCommitIndex() uint64 {
	if m != nil {
		return m.CommitIndex
	}
	return 0
}

func init() {
	proto.RegisterType((*SnapshotRecoveryResponse)(nil), "protobuf.SnapshotRecoveryResponse")
}

func init() {
	proto.RegisterFile("snapshot_recovery_response.proto", fileDescriptor_snapshot_recovery_response_01c34a964a4060da)
}

var fileDescriptor_snapshot_recovery_response_01c34a964a4060da = []byte{
	// 140 bytes of a gzipped FileDescriptorProto
	,
}
